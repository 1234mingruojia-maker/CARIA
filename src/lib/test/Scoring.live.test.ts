...
function answers(overrides: Record<string, number> = {}): Record<string, number> {
  // key ครอบคลุมทุก column ที่มีใน questionMap
  // ใช้ชื่อ dbColumn ตรงๆ
...
// ตรวจว่า DB มีข้อมูลพร้อมใช้จริง
// ─────────────────────────────────────────────────────────────
...
describe('[LIVE] recommendCareers — ผลลัพธ์พื้นฐาน', () => {
  ...
  it('mesScore ทุกตัวอยู่ในช่วง 0–1', async () => {
    const result = await recommendCareers({}, answers(), undefined)
    result.top10.forEach(r => {
      expect(r.mesScore, `${r.name} mesScore เกิน range`)
        .toBeGreaterThanOrEqual(0)
      expect(r.mesScore, `${r.name} mesScore เกิน 1`)
  ...
  it('กรอง sector DT ได้ถูกต้อง', async () => {
    ...
    if (careers.length === 0) {
      console.warn('⚠️  ไม่มีอาชีพ sector DT ใน DB — ข้ามเทสนี้')
      return
    }

    const result = await recommendCareers({}, answers(), 'DT')
    console.log(`\n🔍  DT sector: พบ ${result.totalEvaluated} อาชีพ`)
  ...
  it('กรอง sector DC ได้ถูกต้อง', async () => {
    ...
    if (careers.length === 0) {
      console.warn('⚠️  ไม่มีอาชีพ sector DC ใน DB — ข้ามเทสนี้')
      return
    }

    const result = await recommendCareers({}, answers(), 'DC')
    console.log(`\n🔍  DC sector: พบ ${result.totalEvaluated} อาชีพ`)
...
// ตรวจว่า scoring ตอบสนองต่อ input ที่ต่างกัน
// ─────────────────────────────────────────────────────────────
describe('[LIVE] Sensitivity — scoring ตอบสนองต่อ input', () => {
  ...
  it('เปลี่ยนโปรไฟล์ → อันดับ 1 เปลี่ยนหรือ score เปลี่ยน', async () => {
    // โปรไฟล์ A: เน้น programming + math
    ...
    // ต้องได้ผลต่างกัน (อาชีพอันดับ 1 ต่างกัน หรือ score ต่างกันอย่างน้อย 0.01)
    const different =
      resA.top10[0]?.careerId !== resB.top10[0]?.careerId ||
      Math.abs(resA.top10[0]?.mesScore - resB.top10[0]?.mesScore) > 0.01
  ...
  it('เพิ่ม skill ที่ขาดอยู่ → MES ของอาชีพที่ต้องการ skill นั้น ต้องสูงขึ้น', async () => {
    ...
    if (!topCareer) {
      console.warn('⚠️  ไม่มีผลลัพธ์ — ข้ามเทสนี้')
      return
    }

    // หา top gap ของอาชีพอันดับ 1
    const biggestGapKey = Object.entries(topCareer.topGaps)
      .sort((a, b) => b[1] - a[1])[0]?.[0]

    if (!biggestGapKey) {
      console.warn(`⚠️  ${topCareer.name} ไม่มี gap — ข้ามเทสนี้`)
      return
    }
    ...
    // ตอบ key นั้น = 100
    const improvedAnswers = answers({ [biggestGapKey]: 100 })
    ...
    console.log(`   MES หลังเพิ่ม skill: ${improvedCareer?.mesScore}`)
...
describe('[LIVE] topGaps — ข้อมูล gap จาก DB จริง', () => {
  ...
  it('topGaps มีไม่เกิน 5 รายการทุกอาชีพ', async () => {
    ...
    result.top10.forEach(r => {
      expect(
        Object.keys(r.topGaps).length,
        `${r.name} มี topGaps มากกว่า 5`
  ...
  it('topGaps มีเฉพาะค่าบวก (career > student)', async () => {
    ...
    result.top10.forEach(r => {
      Object.entries(r.topGaps).forEach(([key, val]) => {
        expect(val, `${r.name}.topGaps.${key} = ${val} ไม่ใช่ค่าบวก`).toBeGreaterThan(0)
      })
    })
  })
  ...
  it('แสดง gap สรุปของ top 3 อาชีพ', async () => {
    ...
    // เทสนี้ pass เสมอ — ไว้ดู output อย่างเดียว
