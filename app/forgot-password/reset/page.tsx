  ...
  return (
    <main style={{ minHeight: '100vh', background: 'var(--bg)', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '1.5rem', fontFamily: 'var(--font-body)' }}>
      ...
      <div style={{ background: 'var(--card)', borderRadius: '20px', padding: '2.5rem 2rem', width: '100%', maxWidth: 380, boxShadow: '0 4px 20px rgba(44,41,39,0.08)' }}>
        <h1 style={{ fontFamily: 'var(--font-hand)', fontSize: '2rem', textAlign: 'center', marginBottom: '1.75rem', color: 'var(--dark)' }}>Reset Password</h1>
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
          <div style={{ position: 'relative' }}>
            <input style={inputStyle} placeholder="Password" type={showPw ? 'text' : 'password'} value={password} onChange={e => setPassword(e.target.value)}/>
            <button onClick={() => setShowPw(!showPw)} style={{ position: 'absolute', right: 10, top: '50%', transform: 'translateY(-50%)', background: 'none', border: 'none', cursor: 'pointer', color: 'var(--mid)' }}>👁</button>
          </div>
          {password && (
            <div>
          ...
          <div style={{ position: 'relative' }}>
            <input style={inputStyle} placeholder="Confirm Password" type={showConfirm ? 'text' : 'password'} value={confirm} onChange={e => setConfirm(e.target.value)}/>
            <button onClick={() => setShowConfirm(!showConfirm)} style={{ position: 'absolute', right: 10, top: '50%', transform: 'translateY(-50%)', background: 'none', border: 'none', cursor: 'pointer', color: 'var(--mid)' }}>👁</button>
          </div>
